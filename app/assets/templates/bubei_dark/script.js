...
(function () {
  ...
  // ---------- 拼写轮（一轮走完触发，整轮循环在模板里跑） ----------
  var SR_MAX_TRIES = 3;   // 三次机会；点「提示」也算一次
  var srOn = false;
  ...
  var srPending = null;   // 'skip' | 'forget' —— 揭晓答案后等「继续」
  ...
  // 拼写轮：隐藏输入框收键盘，字符逐格画到 .fc-sr-slots
  var srInputEl = root.querySelector(".fc-sr-input");
  if (srInputEl) {
  ...
  //  拼写轮
  //  ------------------------------------------------------------
  //  一轮（复习段 / 新学段）走完后由原生弹窗触发。三种形态：
  //    有例句        -> 句子拼写（例句挖空，拼文中那个变形词）
  //    没例句        -> 单独拼写（只给中文释义）
  //    在语篇里出现  -> 语篇拼写（排最后，语篇原文挖空）
  //  真·逐格输入；跳过 / 提示 / 忘记了；
  //  三次机会用完 = 忘记，放回队尾重来。
  ...
  function srStart(items) {
    ...
    srRevealed = false;
    srOn = srTotal > 0;
    var sec = srQ(".fc-spellround");
    if (sec) sec.hidden = !srOn;
    if (!srOn) { if (FC.spellDone) FC.spellDone(); return; }
    srNext();
  }
  ...
  function srRender() {
    ...
    // 题干：把 ______ 换成格子容器；没挖空就单独起一行放格子
  ...
  root.addEventListener("click", function (e) {
    ...
    // 拼写轮开着时，所有点击都归它 —— 别让底下的卡片逻辑抢事件
    if (srOn) {
      var srBtn = e.target.closest("[data-sr]");
      if (srBtn) {
        e.preventDefault();
        e.stopPropagation();
        srAction(srBtn.getAttribute("data-sr"));
        return;
      }
