...
class TtsDiagnostics {
  static Future<String> run() async {
    ...
    try {
      ...
      line('speak: $sr');
      await TtsLog.write('diag', 'speak -> $sr');
      line('');
      line('没听到声音 = 引擎没绑上 / en-US 语音数据没下 / 被系统静音策略掐了。');
    } catch (e, st) {
      line('ERROR: $e');
      await TtsLog.write('diag', 'ERROR: $e\n$st');
    } finally {
      try {
        await tts.stop();
      } catch (_) {}
    ...
    await TtsLog.write('diag', '--- report ---\n$report');
    return report;
  }
}
