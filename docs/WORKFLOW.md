...
【重测轮 retest】池内词，每词两个考法
   choice(英→中选义) → cloze(例句挖空选英词)
   两个全过 → 毕业进周期
   任一错 → 留池，下一轮重考（循环到过）
                      │
【拼写轮 spell】一轮（复习段 / 新学段）走完**可选**进入，不强制
   ├─ 句子拼写（有例句，挖空拼文中变形词）
   ├─ 单词拼写（没例句，只给中文释义）
   └─ 语篇拼写（语篇里出现过的词，压最后）
   逐格输入 · 跳过/提示/忘记了 · 三次机会用完=忘记放回队尾
   ★ **不写 FSRS**，轮内循环到全过（或点「结束拼写」退出）
                     │
【复习轮 review】到期词
   同套流程 → FSRS 更新 → 排下次（间隔越来越长）
                     │
【任意时刻】☆收藏 ｜ 熟=标熟 ｜ abc=随手拼 ｜ ⋯ 更多
                     │
...
|---|---|---|
| ☆ 收藏 | 进全局生词本，跨书 | 待做 |
| 熟 | 标熟 = 永久出队（可撤销） | ✅ 已做（`afc1355`） |
| abc | 当场拼写测验（随手练，一个词） | ✅ 已做（`afc1355`） |
| ⋯ | 编辑释义 / 加笔记 / AI 问词 / 看真题出处 / 移入自定义词单 | 待做 |

**另有「拼写轮」**（不在顶栏，一轮走完弹窗问）：逐格输入 / 三种形态 /
三次机会循环到全过，**不写 FSRS**。见 `text/worklogs/2026-09-16-早场.md`。
